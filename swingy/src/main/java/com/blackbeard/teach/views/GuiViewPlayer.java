...
import static com.blackbeard.teach.views.colors.ANSI_GREEN;
...
public class GuiViewPlayer extends PlayerView implements WindowManager {
	...
	public GuiViewPlayer() {
		super();
		returnNum = 3;
		frame = new JFrame("Welcome To Swingy");
	...
	public int	choosePlayer(PlayerController controller) {
		...
		pnl.add(Box.createRigidArea(new Dimension(250, 50)));
		pnl.add(btnNew);
		pnl.add(Box.createRigidArea(new Dimension(250, 50)));
		pnl.add(btnSelect);
		pnl.add(Box.createRigidArea(new Dimension(250, 50)));
	...
	private void savePlayerFields() {
		...
		if (this.playerController.savePlayer()) {
			JOptionPane.showMessageDialog(frame, ANSI_GREEN+"HERO SAVED");
			frame.setVisible(false);
			frame.dispose();
